...
class departementSpider(scrapy.Spider):
    name = 'departement'
    custom_settings = {
        'ITEM_PIPELINES': {'amazonSpider.pipelines.departPipeline': 800}
    }
    ...
    def parse(self, response):
        ...
        for p in response.xpath('//*[@id="zg-ordered-list"]/li'):
            ...
            if prix == None and petit_prix == None and grand_prix == None:
                if 'Livres' in category:
                    item['prix'] = p.xpath('span/div/span/div[@class="a-row"]/a/span/text()').extract_first()
                else:
...
class AmazonSpider(scrapy.Spider):
    ...
    custom_settings = {
        'ITEM_PIPELINES': {'amazonSpider.pipelines.AmazonspiderPipeline': 300},
    }
    ...
    def parse(self, response):
        ...
        for p in response.xpath('//*[@id="zg-ordered-list"]/li'):
            ...
            if prix == None and petit_prix == None and grand_prix == None:
                if 'Livres' in category:
                    item['prix'] = p.xpath('span/div/span/div[4]/a/span/text()').extract_first()
                else:
                    text = p.xpath('span/div/span/a[@class="a-link-normal a-text-normal"]/span/text()').extract_first()
                    pr = p.xpath('span/div/span/a[@class="a-link-normal a-text-normal"]/span/span/span/text()').extract_first()
                    if text != None:
